	    ...
	    int verbosity = 0)
{
  ...
  double svxrad[4] = {svtx_inner_radius, 3.2, 3.9, 65.0}; // ALICE ITS upgrade with outer layer pushed out
  double si_thickness[4] = {0.005, 0.005, 0.005, 0.005};  // ALICE ITS upgrade Si thickness is 50 microns
  double length[4] = {27.0, 27.0, 27.0, -1.}; // -1 use eta coverage to determine length - modified from ITS
  ...
  double support_thickness[4] = {0.0036, 0.0036, 0.0036, 0.0108};
...
void Svtx_Reco(int verbosity = 0)
{
  ...
  //---------------------
  PHG4HoughTransform* hough = new PHG4HoughTransform(4,4);
  ...
  // Ghost rejection
  //---------------------
  PHG4TrackGhostRejection* rejection = new PHG4TrackGhostRejection(4);
  rejection->Verbosity(verbosity);
  rejection->set_max_shared_hits(2);
